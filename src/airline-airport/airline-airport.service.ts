...
export class AirlineAirportService {
  ...
  async findAirportByAirlineIdAirportId(
    ...
    const { airport } = await this.findAirportInAirline(airlineId, airportId);
  ...
  async deleteAirportAirline(airlineId: string, airportId: string) {
    const { airline } = await this.findAirportInAirline(airlineId, airportId);

    airline.airports = airline.airports.filter((e) => e.id !== airportId);
    await this.airlineRepository.save(airline);
  }

  private async findAirportInAirline(
    airlineId: string,
    airportId: string,
  ): Promise<{
